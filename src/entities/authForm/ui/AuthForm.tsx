...
export const AuthForm = ({
  ...
  return (
    <FormWrapper action={action} className={formStyles.authForm}>
      <>
        <Input
          label="E-mail"
          type="email"
        ...
        {isLoginMode && (
          <div className="flex justify-center mt-[4px]">
            <a href="/auth/restore" className="underline">
              Забыли пароль?
            </a>
          </div>
        )}
        <Button
          type="submit"
          disabled={isPending}
          className={`right-arrow ${isLoginMode ? "mt-[16px]" : "mt-[4px]"}`}
        >
          <p>{isRegistrationMode ? "Регистрация" : "Войти в кабинет"}</p>
        </Button>
        {errors?.general && (
          <div className="error text-center mt-[8px]">{errors.general}</div>
        )}
