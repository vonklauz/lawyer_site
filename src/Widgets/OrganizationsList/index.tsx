...
export const OrganizationsList = () => {
  ...
  const getTitle = (entityName: keyof Entities) => {
    ...
    return (
      <div className="flex justify-between items-center">
        <div>{entityTypeTitle}</div>
        <Link
          className="ml-3 text-xs underline!"
          href={linkToCreateNewEntityTypeItem}
        >
          Добавить
        </Link>
      </div>
  ...
  const getOptions = (entities: Entities, entityName: keyof Entities) => {
    ...
    entitiesArr.forEach(
      ...
      ({ entity_id, name, last_name, first_name, middle_name }) => {
        mappedOptions.push({
          id: entity_id,
          name:
            entityName === "company"
              ? name
              : entityName === "sole_proprietor"
        ...
        Object.keys(entities).map((entityName) => {
          return (
            <Fragment key={entityName}>
              <Table
                title={getTitle(entityName as keyof Entities)}
                structure={tableStructure}
                data={getOptions(entities, entityName as keyof Entities)}
                colorScheme={entityName === "company" ? "red" : "blue"}
              />
              <Gap size={20} />
            </Fragment>
          );
