...
export const RequisitesForm: FC<RequisitesFormProps> = ({
  entityType,
  entityId,
}) => {
  ...
  const { mutate: createEntityRq, isPending: isCreatingEntity } = requestData;
  const { mutate: updateEntityRq, isPending: isUpdatingEntoty } = updateData;
  const requestDataResponse = (requestData.data || updateData.data) as
    | EntityResponse
    | undefined;
  ...
  const [schemaResponse, isSchemaLoading] =
    useInterceptor<SchemaResponse>(getSchema);
  const isLoading =
    isCreatingEntity || isUpdatingEntoty || (isSchemaLoading as boolean);
  ...
  useEffect(() => {
    ...
    if (requestDataResponse?.success) {
      ...
      if (entityId) {
        const index = newEntities[entityType].findIndex(
  ...
  useEffect(() => {
    if (hasHydrated && entityId) {
      const prefilledForm = entities[entityType].find(
        (item) => item.entity_id === entityId,
      );
      if (prefilledForm) {
  ...
  const validateAndSend = (requestData: EntityFormData): void => {
    ...
    const validationSchema = mapSchemaFromData(
      fields.length > 0 ? [fields] : [[] as any],
    );
    try {
