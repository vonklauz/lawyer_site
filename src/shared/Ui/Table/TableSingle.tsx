...
export const TableSingle = ({
  ...
  const filteredData =
    filter === "all" ? data : data?.filter((item) => item.status === "new");
    ...
    <div className="overflow-x-scroll md:overflow-x-auto">
      <div className={styles["table-container"]}>
        <div className={styles["table-filter"]}>
          <h4 className={`${styles["table-title"]} ${colorScheme}-bg`}>
            {title}
          </h4>
        ...
        <table className={styles["data-table"]}>
          ...
          <tbody>
            {isLoading ? (
              <tr>
                <td
                  // @ts-expect-error кол-во колонок может быть разным, позже типизировать
              ...
              filteredData?.map(
                ({ id, name, submissionDate, status, detailsUrl }) => {
                  const is2ColTable = !submissionDate && !status;
                  return (
                    <tr key={id}>
                      <td className={`${is2ColTable ? "w-[70%]" : ""}`}>
                        {name}
                      </td>
