...
const getMessageForCyrillic = (fieldname: string): string =>
  `Поле "${fieldname}" должно содержать только кириллические символы`;
const getMinLengthMessage = (fieldname: string): string =>
  `Поле "${fieldname}" должно содержать не менее 3 символов`;
const getMaxLengthMessage = (fieldname: string): string =>
  `Поле "${fieldname}" должно содержать не более 30 символов`;
...
export const registerSchema = object({
  ...
  phone: string()
    .matches(PHONE_REGEXP, "Номер телефона должен быть формата +7XXXXXXXXXX")
    .length(12)
    .required(),
}).concat(loginSchema);
 ...
 */
export const mapSchemaFromData = (data: [][]) => {
  const schema: ObjectWithProps = {};

  data.forEach((fields) => {
    fields.forEach(({ name, required, length, title }) => {
